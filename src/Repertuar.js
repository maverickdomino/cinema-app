			...
			<div className="table-container">
					<table>
						<tbody>
							<tr>
								<th>Title</th>
								{ this.state.repertoireDays.map(day => <th key={day}>{day}</th>) }
							</tr>
								{ this.state.titles.map((title) => <tr key={title}><th>{title}</th>
									{ this.state.repertoireDays2.map(day => <td key={day}>
...
class MovieHours extends Component {
	...
	render() {
		...
		return (
			<div>
				{this.props.titleAll.map((title,index) => (this.props.repertoireDay === this.props.movieDay[index]) && (this.props.title === title) ? <div key={index} onClick={() => this.handleClick(index)}><Link to={`/${this.props.ids[index]}`}>{this.props.hour[index]}</Link></div> : '')}
			</div>
		);
	}
}
